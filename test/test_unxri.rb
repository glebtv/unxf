...
class TestUnXRI < Test::Unit::TestCase
  ...
  def test_ipv6_localhost
    req = Rack::MockRequest.new(UnXF.new(@app))
    env = {
      "HTTP_X_REAL_IP" => "2600:3c01::f03c:91ff:fe96:f5d6",
      "REMOTE_ADDR" => "::1",
    }
    r = req.get("http://example.com/", @req.merge(env))
    assert_equal 200, r.status.to_i
  ...
  def test_ipv6_mixed_localhost
    req = Rack::MockRequest.new(UnXF.new(@app))
    env = {
      "HTTP_X_REAL_IP" => "2600:3c01::f03c:91ff:fe96:f5d6,192.168.1.1",
      "REMOTE_ADDR" => "127.0.0.1",
    }
    r = req.get("http://example.com/", @req.merge(env))
    assert_equal 200, r.status.to_i
    assert_equal "2600:3c01::f03c:91ff:fe96:f5d6", @env["REMOTE_ADDR"]
    assert ! @env.key?("HTTP_X_REAL_IP")
    assert_equal "2600:3c01::f03c:91ff:fe96:f5d6,192.168.1.1", @env["unxf.for"]
  end

  def test_multiple_proxies
    req = Rack::MockRequest.new(UnXF.new(@app))
    env = {
      "HTTP_X_REAL_IP" => "0.6.6.6,192.168.1.1",
      "REMOTE_ADDR" => "127.0.0.1",
    }
    r = req.get("http://example.com/", @req.merge(env))
    assert_equal "0.6.6.6", @env["REMOTE_ADDR"]
  ...
  def test_trusted_chain
    req = Rack::MockRequest.new(UnXF.new(@app))
    env = {
      "HTTP_X_REAL_IP" => "0.6.6.6,192.168.0.1",
      "REMOTE_ADDR" => "127.0.0.1",
    }
    r = req.get("http://example.com/", @req.merge(env))
    assert_equal 200, r.status.to_i
  ...
  def test_spoofed_in_chain
    req = Rack::MockRequest.new(UnXF.new(@app))
    env = {
      "HTTP_X_REAL_IP" => "0.6.6.6,8.8.8.8",
      "REMOTE_ADDR" => "127.0.0.1",
    }
    r = req.get("http://example.com/", @req.merge(env))
  ...
  def test_more_trust
    ...
    env = {
      "HTTP_X_REAL_IP" => "1.6.6.6,0.6.6.6",
      "REMOTE_ADDR" => "127.0.0.1",
    }
    r = req.get("http://example.com/", @req.merge(env))
    assert_equal r.status.to_i, 200
